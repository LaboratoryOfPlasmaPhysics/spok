...
class TestMagnetosheath(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.typical_msh = Magnetosheath(
            magnetopause="mp_shue1998", bow_shock="bs_jelinek2012"
        )
        theta = np.arange(0, np.pi / 2, 0.01 * np.pi)
        self.typical_mp_sph, self.typical_bs_sph = self.typical_msh.boundaries(
            theta, 0, coord_sys="spherical"
        )
        self.typical_mp_cart, self.typical_bs_cart = self.typical_msh.boundaries(
            theta, 0, coord_sys="cartesian"
        )
    ...
    @data(
        ("bs_shue1998", "mp_formisano1979"),  # both wrong
        ("mp_shue1998", "mp_formisano1979"),
    )  # bow shock wrong
    @unpack
    def test_msh_invalid_boundaries_raise(self, mp, bs):
        """Test something."""
        self.assertRaises(ValueError, Magnetosheath, magnetopause=mp, bow_shock=bs)
    ...
    def test_spherical_and_cartesian_are_consistent(self):
        for model_sph, model_cart in zip(
            (self.typical_mp_sph, self.typical_bs_sph),
            (self.typical_mp_cart, self.typical_bs_cart),
        ):
            rfromcart = np.sqrt(
                model_cart[0] ** 2 + model_cart[1] ** 2 + model_cart[2] ** 2
            )
